...
<?php
	...
	$query_rsSimpleGameSearch = $queryString;
	$rsSimpleGameSearch = mysql_query($query_rsSimpleGameSearch, $GamestorePOS) or die(mysql_error());
	$row_rsSimpleGameSearch = mysql_fetch_assoc($rsSimpleGameSearch);
	$totalRows_rsSimpleGameSearch = mysql_num_rows($rsSimpleGameSearch);mysql_select_db($database_GamestorePOS, $GamestorePOS);
...
<table width="100%" border="1">
  <tr>
    ...
    <td><?php echo $fields[$tempDoTableTitleRow]; ?></td>
    <?php $tempDoTableTitleRow++; } while ($tempDoTableTitleRow < $noOfFields) ?>
  </tr>
  <?php do { ?>
    <tr> 
    	<?php $tempDoTable = 0; do { ?>
    	<td><?php echo $row_rsSimpleGameSearch[$fields[$tempDoTable]]; ?></td>
    	<?php $tempDoTable++; } while ($tempDoTable < $noOfFields) ?>
    </tr>
    <?php } while ($row_rsSimpleGameSearch = mysql_fetch_assoc($rsSimpleGameSearch)); ?>
</table>
